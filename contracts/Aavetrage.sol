...
contract Aavetrage {
    ...
    address constant collateralAsset = address(0x99b267b9D96616f906D53c26dECf3C5672401282);                                // Kovan AAVE Token
    address public bestBorrowToken;
    address public bestSupplyToken;
    uint256 public collateralAmount = 100000000000000000;

    ILendingPool lendingPool;
    ILendingPoolAddressesProvider provider;

    event Peek(bool arbitrageIdentified);
    event Guap(address collateralAsset, bool stored);
    ...
    /*
        ...
        True iff we have identified an arbitrage opportunity between two distinct markets
    */
    ...
    */
    function guap() external {
        ...
        require(IERC20(collateralAsset).allowance(msg.sender, address(this)) >= collateralAmount, "Allowance criteria for Aavetrage not met...");
        ...
        require(IERC20(collateralAsset).allowance(address(this), provider.getLendingPool()) >= collateralAmount, "Allowance criteria for LendingPool not met...");

        // deposit into lending pool our designated amount, aTokens received by contract
        lendingPool.deposit(collateralAsset, collateralAmount, address(this), 0);
        
        // set the deposit to be used as collateral
        lendingPool.setUserUseReserveAsCollateral(collateralAsset, true);
        
        // uint256 borrowAmount = collateralAmount - 10000;
        IERC20(bestBorrowToken).safeApprove(address(this), 10000);
        
        // borrow the best borrow token against the value of our collateral, let debt be incurred by contract
        lendingPool.borrow(bestBorrowToken, 10000, 2, 0, address(this));
